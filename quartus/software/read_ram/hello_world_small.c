...
// Función para leer un byte desde UART
...
void uart_send_char(char c) {
    																// Esperar hasta que el UART esté listo para transmitir
    while (!(IORD(UART_DATA_BASE, 2) & 0x40)) {						// Bit 6 (Transmit Ready) del registro de estado debe ser 1
    }
    IOWR(UART_DATA_BASE, 1, c); 									// Escribir el carácter en el registro de datos (OFFSET 1)
}
...
int main()
{
	...
	alt_u32 led_value = 0;
	 ...
	 while (1) {
	        ...
	        button_value = IORD_ALTERA_AVALON_PIO_DATA(BUTTON_BASE);			// leer el valor del botón
	        if (button_value == 1 || received_data == 32) {											// si el botón está presionado (asumiendo lógica activa baja)
